...
def Block(img_path,num,row_patch_size,col_patch_size,save_path):
    ...
    for i in range(row-1):
        for j in range(col-1):
            ...
            GRID.write_img(save_path+ '/'+str(i)+'_'+str(j)+'.tif', proj, geotrans, block_data)
    for i in range(row-1):
        block_data = img_use[:, (interval_row * i):((i + 1) * interval_row + 2 * row_patch_size),(interval_col * (col-1)):]
        GRID.write_img(save_path+ '/' +str(i)+'_'+str(col-1)+'.tif', proj, geotrans, block_data)
    for j in range(col-1):
        block_data = img_use[:, (interval_row * (row-1)):,(interval_col*j):((j +1) * interval_col+2*col_patch_size)]
        GRID.write_img(save_path+ '/' +str(row-1)+'_'+str(j)+'.tif', proj, geotrans, block_data)
    block_data = img_use[:, (interval_row * (row-1)):,(interval_col * (col-1)):]
    GRID.write_img(save_path+ '/'+str(row-1)+'_'+str(col-1)+'.tif', proj, geotrans, block_data)
    print('分块完成！共%d块' % num)
    end = time()
    time = end - start
    print("分块用时%fs" % time)
...
def merge(num,save_path,row_patch_size,col_patch_size,model_type):
    ...
    for i in range(row):
        ...
        proj, geotrans, A = GRID.read_img(path1)
        ...
        for j in range(1,col):
            ...
            proj, geotrans, B = GRID.read_img(path)
    ...
    GRID.write_img(save_path+'/'+str(model_type)+'_LSM.tif', proj, geotrans, merge_data)
...
def predict(img_dir,model_type,model_save_dir,class_img_dir,row_patch_size,col_patch_size,step):
    ...
    proj, geotrans, data = GRID.read_img(img_dir)
    print(data.shape)
    ...
    GRID.write_img(class_img_dir, proj, geotrans, img_new)
    end = time()
    time = end - start
    print("用时%fs" % time)
    print("滑坡易发性制图完成！")
